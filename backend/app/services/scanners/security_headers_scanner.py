...
class SecurityHeaderObservationService:
    @staticmethod
        ...
        async with httpx.AsyncClient(verify=False, follow_redirects=False, timeout=5.0) as client:
            try:
                ...
                obs_in = ObservationCreate(
                    ...
                    asset_id=asset_id,
                    observation_type=ObservationType.WEAK_HEADER,
                    title=f"Potential Weak Security Headers at {base_url}",
                    fingerprint=f"weak_header_{base_url}"
                )

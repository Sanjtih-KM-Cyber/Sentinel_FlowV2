...
class AdminPanelScanner:
    ...
    async def scan(db: Session, org_id: uuid.UUID, project_id: uuid.UUID, asset_id: uuid.UUID, base_url: str, user: User) -> list:
        ...
        async with httpx.AsyncClient(verify=False, follow_redirects=False, timeout=5.0) as client:
            for path in AdminPanelScanner.TARGET_PATHS:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        obs_in = ObservationCreate(
                            ...
                            asset_id=asset_id,
                            observation_type=ObservationType.ADMIN_PANEL,
                            title=f"Potential Admin Panel Exposed: {path}",
                            fingerprint=f"admin_panel_{path}"
                        )
